...
class MapExtensionTutorialActivity : TutorialActivity<ActivityExplorationMapRepresentationTutorialBinding>(), RobotLifecycleCallbacks {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.extendMapButton.setOnClickListener {
    ...
    override fun onResume() {
        ...
        binding.startMappingButton.isEnabled = false
        binding.extendMapButton.isEnabled = false
        initialExplorationMap = null
        binding.mapImageView.setImageBitmap(null)
    }
    ...
    override fun onRobotFocusGained(qiContext: QiContext) {
        ...
        runOnUiThread {
            binding.startMappingButton.isEnabled = true
        }
    }
    ...
    private fun startMappingStep(qiContext: QiContext) {
        ...
        mapSurroundings(qiContext).thenConsume { future ->
            if (future.isSuccess) {
                displayLine(
                    "Mapping done, retrieving map representation...",
                ...
                runOnUiThread {
                    if (lifecycle.currentState.isAtLeast(Lifecycle.State.RESUMED)) {
                        displayMap(bitmap)
                        binding.extendMapButton.isEnabled = true
                    }
                }
                ...
                if (future.hasError()) {
                    displayLine(
                        "Mapping failed: ${future.errorMessage}",
    ...
    private fun startMapExtensionStep(initialExplorationMap: ExplorationMap, qiContext: QiContext) {
        ...
        extendMap(initialExplorationMap, qiContext) { updatedMap ->
            ...
            if (!future.isSuccess) {
                if (future.hasError()) {
                    displayLine(
                        "Map extension failed: ${future.errorMessage}",
                        ConversationItemType.ERROR_LOG
                    )
                }
                runOnUiThread {
    ...
    private fun displayLine(text: String, type: ConversationItemType) {
        runOnUiThread {
            if (lifecycle.currentState.isAtLeast(Lifecycle.State.RESUMED)) {
                binding.conversationView.addLine(text, type)
            }
        }
    }
}
